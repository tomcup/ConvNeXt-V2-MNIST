...
class Trainer:
    def __init__(
        ...
        self.train_sampler = ResumableRandomSubsetSampler(
            total_size=len(train_dataset), num_samples=self.train_samples_per_epoch
        )
    ...
    def train(self):
        ...
        if self.resume_from:
            ...
            print(
                f"Resumed from epoch {self.current_epoch}, best_val_acc={self.best_val_acc:.4f}"
            )
        ...
        for epoch in range(self.current_epoch, self.epochs):
            ...
            if (epoch + 1) % self.val_every == 0 or epoch == self.epochs - 1:
                ...
                print(
                    f"Epoch {epoch + 1:03d} | Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.4f}"
                )
                ...
                if val_acc > self.best_val_acc:
                    ...
                    save_checkpoint(
                        save_path=os.path.join(
                            self.checkpoint_dir, f"epoch_{epoch + 1}"
                        ),
                        model=self.model,
                ...
                if self.logger.writer:
                    self.logger.writer.add_scalar(
                        "train/loss", avg_train_loss, epoch + 1
                    )
                    self.logger.writer.add_scalar(
                        "train/lr", self.optimizer.param_groups[0]["lr"], epoch + 1
                    )
                    if val_loss is not None:
                        self.logger.writer.add_scalar("val/loss", val_loss, epoch + 1)
                        self.logger.writer.add_scalar(
                            "val/accuracy", val_acc, epoch + 1
                        )

                    # 取一个 batch 的图像并记录到 tensorboard
                    sample_batch = next(iter(train_loader))[0][:32]  # 前 32 张
                    grid = torchvision.utils.make_grid(
                        sample_batch.cpu(), nrow=8, normalize=True
                    )
